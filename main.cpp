...
int main(int argc, char *argv[]){
	...
	if(argc < 3){
		qInfo() << "Usage:" << argv[0] << "config.json" << "file.pcap";
		return(-1);
	}
	...
	queueInactiveInterval = config.value("queueInactiveInterval").toInt();
	...
	tshark.start("tshark",QStringList() << "-o" << "tcp.desegment_tcp_streams:false" << "-M" << "100000" << "-l" << "-T" << "ek" << "-n" << "-r" << argv[2] << tsharkFields);
